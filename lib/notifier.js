...
async function notifierUsersProches(incident) {
  try {
    ...
    const isAcc = incident.type === 'accident';
    const titre = isAcc ? '🚨 Accident signalé' : '🚦 Bouchon détecté';
    ...
    const messages = users
      .filter(u =>
        ...
        distKm(incident.latitude, incident.longitude, u.latitude, u.longitude) <= 2
      )
      .map(u => ({
        to: u.push_token,
        title: titre,
        ...
        sound: 'default',
        badge: 1,
        data: { incidentId: incident.id, type: incident.type, lat: incident.latitude, lng: incident.longitude },
        priority: 'high',
        channelId: 'traficam',
      }));
