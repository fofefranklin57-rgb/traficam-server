...
// Détection bouchon automatique (état en DB pour serverless)
async function analyserTrafic(latitude, longitude, vitesse) {
  if (vitesse >= 10) return null;

  // Compter les positions lentes récentes dans rayon 300m (2 dernières minutes)
  const { data: positionsLentes } = await supabase
    .from('positions_gps')
    .select('latitude, longitude')
    .gte('created_at', new Date(Date.now() - 2 * 60000).toISOString())
    .lte('vitesse', 10);

  const lentesProches = (positionsLentes || []).filter(p =>
    distKm(latitude, longitude, p.latitude, p.longitude) < 0.3
  );

  if (lentesProches.length < 3) return null;

  // Vérifier qu'il n'y a pas déjà un bouchon récent dans ce secteur
  const { data: bouchonsProches } = await supabase
    .from('incidents')
    .select('id')
    .eq('statut', 'actif')
    .eq('type', 'bouchon')
  ...
  const dejaSignale = (bouchonsProches || []).some(b =>
    distKm(latitude, longitude, b.latitude, b.longitude) < 0.3
  );
  if (dejaSignale) return null;

  // Créer incident automatique
  const { notifierUsersProches } = require('../../lib/notifier');
  const { data: incident } = await supabase
    .from('incidents')
    .insert([{
      type: 'bouchon', gravite: 'moyen',
      lieu: 'Zone détectée automatiquement',
...
module.exports = async (req, res) => {
  ...
  // Mettre à jour position de l'utilisateur
  if (utilisateur_id) {
    await supabase.from('utilisateurs')
      .update({ latitude, longitude, derniere_activite: new Date() })
      .eq('id', utilisateur_id);
  }

  const incidentAuto = await analyserTrafic(latitude, longitude, vitesse);
  res.json({ success: true, detection_auto: !!incidentAuto, incident: incidentAuto || null });
};
